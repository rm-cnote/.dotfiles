...
| ~SPC~     | Scroll up (page down)           |
| ~DEL~     | Scroll down (page up)           |
| ~M-g g~   | Go to page                      |
| ~o~       | Jump to outline (table of contents) |
| ~C-c C-r~ | Restore last position           |
...
| ~a m~     | Add markup (highlight) annotation |
...
- The ~:magic~ matcher catches PDFs whose filename lacks a ~.pdf~ extension
  (downloads, attachments, temp files).

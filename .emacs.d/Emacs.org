...
Getting this wrong is quiet. The wrapper cannot distinguish "this account has
no token configured" from "the variable never reached me", so it falls back to
the ambient login rather than failing. Check from Emacs:

...
Selection section of =Emacs.org=. Token-based accounts need the
[[https://developer.1password.com/docs/cli/][1Password CLI]] (~op~). ~CLAUDE_ACCT_CLAUDE_BIN~ overrides the path to the real
=claude= binary.
...
- [[https://developer.1password.com/docs/cli/][1Password CLI]] (~op~) — for token-based accounts in ~bin/claude-acct~
